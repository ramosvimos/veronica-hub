...
export const editorialPrimaryNav = ['/', '/news/', '/release-date/', '/gameplay/', '/characters/', '/trailer/', '/sources/', '/watchlist/'];
const kinds = new Set(['guide', 'character', 'news', 'news-index']);
const labels = { confirmed: 'Official remake / announcement', reported: 'Attributed producer report', original: 'Original-game archive', unknown: 'Not established by this source', editorial: 'Reading guide' };
...
export function applyEditorialContent(input, manifest) {
  ...
  for (const p of manifest.pages) {
    ...
    for (const b of p.blocks) {
      ...
      requireValue(b.status === 'editorial' || b.sourceIds?.length, `Missing section evidence: ${p.path}#${b.id}`);
    ...
    requireValue(wordCount >= 150, `Thin editorial content: ${p.path}`);
    const route = { path:p.path, title:p.title, h1:p.h1, navLabel:p.navLabel, description:p.description, intro:p.intro,
      section:'editorial', priority:p.kind === 'news-index' ? '0.8' : '0.7', changefreq:p.kind === 'news-index' ? 'weekly' : 'monthly',
  ...
  if (!data.changelog.some(e => e.date === manifest.publishedAt && e.title === title)) data.changelog.unshift({
    date:manifest.publishedAt, type:'Site Update', title,
...
function cards(routes, {short = false} = {}) {
  return `<div class="editorial-card-grid">${routes.map(r => `<a class="card editorial-card" href="${escapeHtml(r.path)}">
    <span class="eyebrow">${escapeHtml(r.editorial?.kind === 'news' ? 'Source-checked report' : r.editorial?.kind === 'character' ? 'Character dossier' : 'Explore')}</span>
    <h3>${escapeHtml(short ? r.navLabel : r.h1)}</h3><p>${escapeHtml(r.intro)}</p>
    ${r.editorial?.kind === 'news' ? `<time datetime="${r.editorial.publishedAt}">Published ${r.editorial.publishedAt}</time>` : ''}
    <span class="editorial-read">Read ${r.editorial?.kind === 'news' ? 'report' : 'guide'} <span aria-hidden="true">→</span></span>
  </a>`).join('')}</div>`;
}
...
export function editorialSchema(route,data) {
  ...
  const base = {'@context':'https://schema.org','@type':p.kind === 'news-index' ? 'CollectionPage' : p.kind === 'news' ? 'NewsArticle' : 'Article',
  ...
  if (p.kind === 'news-index') base.mainEntity = {'@type':'ItemList',itemListElement:data.routes.filter(r => r.editorial?.kind === 'news').map((r,n) => ({'@type':'ListItem',position:n+1,url:data.site.origin+r.path,name:r.h1}))};
  return base;
}
export function renderEditorialBody(route, data) {
  const p = route.editorial;
  const parent = p.parent && getRoute(data,p.parent);
  const news = data.routes.filter(r => r.editorial?.kind === 'news').sort((a,b) => b.editorial.publishedAt.localeCompare(a.editorial.publishedAt));
  ...
  const sourceList = sourceIds.map(id => {const s=data.sources.find(x => x.id===id);return `<li>${sourceLinks([id],data)} <span class="meta">Checked ${s.lastChecked}</span></li>`;}).join('');
  ...
  return `<div class="editorial-content" data-editorial-path="${escapeHtml(p.path)}">
    <div class="editorial-byline"><span>By ${escapeHtml(p.author)}</span><time datetime="${p.publishedAt}">Published ${p.publishedAt}</time><time datetime="${p.lastReviewed}">Last checked ${p.lastReviewed}</time>${p.eventDate ? `<time datetime="${p.eventDate}">Source announcement ${p.eventDate}</time>` : ''}</div>
